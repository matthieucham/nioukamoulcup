...
def _do_transfer(sale):
    if sale.type == "PA":
        ...
        league_models.Signing.objects.create(
            player=sale.player,
            team=winner,
        ...
        if sale.winning_auction is not None:
            ...
            signing = league_models.Signing.objects.get(
                player=sale.player,
                team=sale.team,
                end__isnull=True,
                league_instance=sale.merkato_session.merkato.league_instance,
            )
            league_models.Signing.objects.end(
                signing, "MV", amount=sale.get_selling_price()
            )
            league_models.BankAccount.objects.sell(sale)
            # start new contract:
            league_models.Signing.objects.create(
                player=sale.player,
                team=sale.winning_auction.team,
                league_instance=sale.merkato_session.merkato.league_instance,
                attributes=_make_signing_attr(sale),
            )
            league_models.BankAccount.objects.buy(sale)
...
def _make_signing_attr(sale):
    return {
        "amount": float(
            sale.get_buying_price().quantize(
        ...
        "release_amount": float(
            (
                sale.get_buying_price()
                * decimal.Decimal(
                    1.0 - sale.merkato_session.merkato.configuration.get("re_tax_rate")
                )
            ).quantize(decimal.Decimal(".1"), rounding=decimal.ROUND_HALF_UP)
        ),
        "locked": False,
        "score_factor": 1.0
        if sale.player.indraftable
        else sale.merkato_session.attributes.get("score_factor", 1.0),
    }
...
def solve_session(merkato_session):
    ...
    if merkato_session.solving > timezone.now():
        raise SaleSolvingException(
            "Merkato session not over yet, solving time is %s" % merkato_session.solving
        )
    # first, apply releases
    for release in transfer_models.Release.objects.filter(
        merkato_session=merkato_session
    ):
        league_models.BankAccount.objects.release(release)
        league_models.Signing.objects.end(
            release.signing, "RE", date=timezone.now(), amount=release.amount
        )
        release.done = True
        release.save()
    with locked_atomic_transaction.LockedAtomicTransaction(
        league_models.BankAccount
    ):  # prevents changes in BankAccount balance during resolution
...
def solve_draft_session(draft_session):
    ...
    if draft_session.closing > timezone.now():
        raise SaleSolvingException(
            "Draft session not over yet, solving time is %s" % draft_session.closing
        )
    app = list()
...
def solve_draft_pick(draft_session_rank, already_picked_players):
    for pick in draft_session_rank.picks.order_by("pick_order").all():
        if pick.player not in already_picked_players:
            if available_for_pa(
                pick.player,
                draft_session_rank.team.division,
...
def _make_draft_signing_attr(draft_pick):
    return {
        "rank": draft_pick.draft_session_rank.rank,
...
def can_register_auction(team, merkato):
    ...
    expired_sessions = merkato.merkatosession_set.filter(
        solving__lt=timezone.now()
    ).count()
    current_pa = transfer_models.Sale.objects.filter(
        merkato_session__merkato=merkato,
        merkato_session__solving__gt=timezone.now(),
        type="PA",
        team=team,
    ).count()
    previous_pas = transfer_models.Sale.objects.filter(
        merkato_session__merkato=merkato,
...
def can_register_pa(team, merkato):
    ...
    if merkato.end < timezone.now():
        return False, "TOO_LATE"
    current_pa_count = transfer_models.Sale.objects.filter(
        merkato_session__merkato=merkato,
        merkato_session__solving__gt=timezone.now(),
        type="PA",
        team=team,
    ).count()
    current_roster = team.signing_set.filter(
        league_instance=merkato.league_instance, end__isnull=True
    ).count()
...
def can_register_mv(team, merkato):
    ...
    if merkato.end < timezone.now():
        return False, "TOO_LATE"
    current_mv_count = transfer_models.Sale.objects.filter(
        merkato_session__merkato=merkato,
        merkato_session__solving__gt=timezone.now(),
        type="MV",
        team=team,
...
def available_for_pa(joueur, division, instance):
    c1 = league_models.Signing.objects.filter(
...
def available_for_mv(joueur, team, instance):
    return (
        Joueur.objects.filter(
            signing__team=team,
            signing__begin__lt=timezone.now(),
...
def solve_transition_session(transition_session):
    ...
    day = league_models.LeagueInstancePhaseDay.objects.get_latest_day_for_phases(
        league_models.LeagueInstancePhase.objects.filter(
            league_instance=li, type="FULLSEASON"
        )
    )[0]
    ...
    for team in league_models.Team.objects.filter(league=li.league):
        choice = team.transitionteamchoice_set.filter(
            transition_session=transition_session
        ).first()
        if choice is None:
            team.attributes["formation"] = dict(
                transition_session.attributes.get("default_formation")
            )
        else:
...
def _fix_signings(team, nb_to_keep, team_tds, instance):
    if (
        nb_to_keep
        < team.signing_set.filter(league_instance=instance, end__isnull=True).count()
    ):
        nb_to_free = (
            team.signing_set.filter(league_instance=instance, end__isnull=True).count()
            - nb_to_keep
        ...
        signings_not_in_list = (
            league_models.Signing.objects.filter(
                team=team, league_instance=instance, end__isnull=True
            )
            .exclude(player__id__in=in_list)
            .all()
        )
        ...
        if nb_to_free > 0:
            ...
            for sg in league_models.Signing.objects.filter(
                team=team, league_instance=instance, end__isnull=True
            ).filter(player__id__in=to_free):
                league_models.Signing.objects.end(sg, "FR")
