...
def compute_comparative_bonuses(all_perfs):
    ...
    for pj in all_perfs:
        ...
        if (
            "note" in pj.details
            and pj.temps_de_jeu >= PLAYTIME["MIN_BONUS"]
            and pj.details["note"] is not None
        ):
            best_by_position[pj.details["equipe"]][pj.joueur.poste] = max(
                pj.details["note"],
                best_by_position[pj.details["equipe"]][pj.joueur.poste],
            )
...
def _compute_bonus(perf, best_note_by_position):
    ...
    # bonus individuel
    for (i, j) in [
        ("PENALSTOP", "penalties_saved"),
        ("GOAL", "goals_scored"),
        ("PENALTY", "penalties_scored"),
        ("PASS", "goals_assists"),
        ("HALFPASS", "penalties_awarded"),
    ]:
    ...
    if (
        ...
        if poste == "G":
            note_to_beat = max(
                best_note_by_position["dom"]["G"], best_note_by_position["ext"]["G"]
            )
        else:
    ...
    if perf.temps_de_jeu >= PLAYTIME["MIN_BONUS"]:
        ...
        if (
            perf.rencontre.resultat[perf.details["equipe"]]["buts_contre"] == 1
            and perf.rencontre.resultat[perf.details["equipe"]]["penos_contre"] == 1
        ):
            earned.update({"HALFCLEANSHEET": 1})
            base += BONUS["COLLECTIVE"]["HALFCLEANSHEET"][poste]
        if (
            perf.rencontre.resultat[perf.details["equipe"]]["buts_pour"] == 3
            and perf.rencontre.resultat[perf.details["equipe"]]["penos_pour"] == 1
        ):
            earned.update({"HALFOFFENSIVE": 1})
            base += BONUS["COLLECTIVE"]["HALFOFFENSIVE"][poste]
        if (
            perf.rencontre.resultat[perf.details["equipe"]]["buts_pour"] == 3
            and perf.rencontre.resultat[perf.details["equipe"]]["penos_pour"] == 0
        ):
