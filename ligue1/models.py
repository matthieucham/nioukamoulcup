...
class SaisonManager(models.Manager):
    def import_from_statnuts(self, statnuts_instance, sn_client, force_import=False):
        ...
        if (
            force_import
            or saison.derniere_maj is None
            or instance_update > saison.derniere_maj
        ):
            for step in statnuts_instance["steps"]:
                Journee.objects.import_from_statnuts(
                    saison,
                    sn_client.get_step(step["uuid"]),
                    sn_client,
...
class JourneeManager(models.Manager):
    def import_from_statnuts(
        self, saison, statnuts_step, sn_client, force_import=False
    ):
        try:
        ...
        journee, created = self.get_or_create(
            sn_step_uuid=statnuts_step["uuid"], saison=saison, defaults=defaults
        )
        step_update = dateutil.parser.parse(statnuts_step["updated_at"])
        if (
            force_import
            or created
            or journee.derniere_maj is None
            or step_update > journee.derniere_maj
        ):
            for meeting in statnuts_step["meetings"]:
                Rencontre.objects.import_from_statnuts(
                    journee,
                    sn_client.get_meeting(meeting["uuid"]),
                    sn_client,
                    force_import=force_import,
                )
...
class Journee(Importe):
    ...
    def save(self, *args, **kwargs):
        if self.pk is not None:
            debut, fin = timezone.make_aware(
                datetime.datetime(2100, 1, 1), timezone.get_default_timezone()
            ), timezone.make_aware(
                datetime.datetime(1998, 1, 1), timezone.get_default_timezone()
            )
            for x in (
                rencontre.date for rencontre in Rencontre.objects.filter(journee=self)
            ):
...
class Club(Importe):
    SVG_TEMPLATES = (
        ...
        ("jersey-diag-half-white2", "moitié diagonale blanche"),
        ("jersey-scap2", "scapulaire"),
        ("jersey-shoulders2", "épaules"),
        ("jersey-halfdark2", "moitié assombrie"),
        ("jersey-stripe-center2", "bande centrale"),
...
class JoueurManager(models.Manager):
    ...
    def get_or_create_from_statnuts(self, statnuts_data):
        defaults = {
            "prenom": statnuts_data["first_name"],
    ...
    def set_club_from_statnuts(self, joueur, statnuts_data_teams, maj, saison=None):
        for t in statnuts_data_teams:
            ...
            try:
                club = Club.objects.filter(participations=saison).get(
                    sn_team_uuid=t["uuid"]
                )
                joueur.club = club
    ...
    def order_queryset_by_poste(self, qs):
        listed = qs.all()
        return sorted(
            listed, key=lambda j: self.POSTE_SORT_ORDER[j.poste] if j.poste else 0
        )
...
class Joueur(Importe):
    ...
    def __str__(self):
        return "%s %s%s" % (
            self.prenom,
            self.nom,
            (" (%s)" % self.surnom if self.surnom else ""),
        )
...
class RencontreManager(models.Manager):
    @timed
    def import_from_statnuts(
        self, journee, statnuts_meeting, sn_client, force_import=False
    ):
        ...
        if (
            force_import
            or created
            or rencontre.derniere_maj is None
            or meeting_update > rencontre.derniere_maj
        ):
    ...
    def _get_or_create_from_statnuts(self, journee, statnuts_data):
        ...
        dom, _ = Club.objects.get_or_create(
            sn_team_uuid=ht_uuid, defaults={"nom": ht_name}
        )
        ext, _ = Club.objects.get_or_create(
            sn_team_uuid=at_uuid, defaults={"nom": at_name}
        )

        defaults = {
            "date": dateutil.parser.parse(statnuts_data["date"]),
            "club_domicile": dom,
            "club_exterieur": ext,
        }
        return self.get_or_create(
            sn_meeting_uuid=statnuts_data["uuid"], journee=journee, defaults=defaults
        )

    def _delete_and_recreate_performances(self, rencontre, statnuts_meeting, sn_client):
        dom_or_ext = {
            statnuts_meeting["home_team"]["uuid"]: "dom",
            statnuts_meeting["away_team"]["uuid"]: "ext",
        }
        rosperfs = []
        ...
        for ros in harmonized_roster:
            ...
            if (
                created
                or joueur.derniere_maj is None
                or joueur.derniere_maj < joueur_updated_at
            ):
                Joueur.objects.set_club_from_statnuts(
                    joueur,
                    sn_client.get_person_teams(ros["player"]["uuid"]),
                    joueur_updated_at,
                    saison=rencontre.journee.saison,
                )
            ...
            if ros["stats"] is None:
                ...
                rosperfs.append(
                    Performance(
                        ...
                        club=club,
                        temps_de_jeu=tps,
                        details=make_performance_details(
                            ros, dom_or_ext[ros["played_for"]]
                        ),
                    )
                )
...
class Rencontre(Importe):
    ...
    def __str__(self):
        if self.resultat is None:
            ...
            return "%s %s-%s %s" % (
                self.club_domicile.nom,
                self.resultat["dom"]["buts_pour"],
                self.resultat["ext"]["buts_pour"],
                self.club_exterieur.nom,
            )
...
def make_rencontre_resultat(statnuts_meeting):
    dom = {
        "buts_pour": statnuts_meeting["home_result"],
        "buts_contre": statnuts_meeting["away_result"],
    }
    ext = {
        "buts_pour": statnuts_meeting["away_result"],
    ...
    for roster in statnuts_meeting["roster"]:
        if (
            roster["stats"] is not None
            and roster["stats"]["penalties_scored"] is not None
            and roster["stats"]["penalties_scored"] > 0
        ):
...
def make_performance_details(statnuts_roster, dom_or_ext):
    return {
        "equipe": dom_or_ext,
        "stats": statnuts_roster["stats"],
        "note": statnuts_roster["hnote"],
    }
