...
class PositionFilter extends Component {
	...
	render() {
		return(
			<form>
			<label>
			Poste: <select onChange={this.props.handleChange} value={this.props.value}>
			<option value=""></option>
...
class PlayersFilterTable extends Component {
	...
	render() {
    	return (
    		<div>
			<PositionFilter value={ this.state.filterPosition } handleChange={ ( event ) => this.setState({ filterPosition: event.target.value }) } />
			<PlayersRankingTable players={ PLAYERS } phases={ PHASES } filterPosition={ this.state.filterPosition } height={ 700 }/>
			</div>
		)
	}
	...
	render() {
		...
		rankings.push(
				<TabContent for={ 'ttab_players' } key={ 'tablink_players'}>
					<PlayersRankingTable players={ playersRanking } phases={ phases } height={ 700 }/>
				</TabContent>
			);
		return (
			<section>
				<h1 className="page-title">Classements après la journée {phases[0].current_ranking.number}</h1>
				<Tabs name="rankingTabs" 
					handleSelect={ (selectedTab, namespace) => { if (selectedTab == 'ttab_players') { onPlayersTab() } ; this.setState({selectedTab: selectedTab}) } }
