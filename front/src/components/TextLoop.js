...
class TextLoop extends React.PureComponent {
    ...
    tick() {
        ...
        if (this.state.currentWord == React.Children.count(this.props.children) -1) {
            if (this.props.stopLoop && this.tickInterval != null) {
                clearInterval(this.tickInterval);
            }
            if (this.props.onLoopCompleted) { this.props.onLoopCompleted(this.state); }
        }
    };
