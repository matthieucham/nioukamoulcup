...
export const SalesTable = ({ sales, height }) => {
		...
		return (<AutoSizer disableHeight>
			{({width}) => (
				...
				cellDataGetter={({rowData}) => Moment(rowData.merkato_session.closing).format('DD/MM/YYYY') } 
				width={120}/>

				<Column	label="Joueur"
				dataKey="player.display_name"
				cellDataGetter={({rowData}) => rowData.player.display_name }
				cellRenderer={({rowData}) => <a href={rowData.player.url}>{rowData.player.display_name}</a> } 
				width={200} flexGrow={1}/>
				
				<Column	label="Mise à prix"
				dataKey="min_price"
