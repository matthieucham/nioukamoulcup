...
class SaleBefore extends React.Component {
	...
	findMyOffer(auctions) {
		for (const auction of auctions) {
			if (auction.is_mine === true)
				return ''+auction.value+' Ka';
		}
		return '-';
	}
...
class SaleDuring extends React.Component {
	...
	render() {
		...
		const pastP = allOffers.slice(0, auctionIndex).map( (auction, index) => {
			...
			return (<p key={'off'+index} className={classes}>{ auction.value }</p>);
		})
		return (
			<div>
				<div className="salecard-content">
					<div className="past-offers">
						{ pastP }
					</div>
				...
				<div className="salecard-actions">
					...
					<Button color="primary" onClick={this.handleRevealClicked}>Révéler</Button>
					<Button variant="contained" color="primary" onClick={this.handleNextClicked}>Suivante</Button>
				</div>
			</div>
		);
	}
			...
			{hasWinner ? (
				<div className="salecard-content">
					<div className="winner">
						<ReactRevealText show={this.state.showWinner} className="salecard-winner">{ winnerName }</ReactRevealText>
					</div>
					<div className="current-offer">
...
class SaleCardContent extends React.Component {
	...
	render() {
		...
		return (
			<CardContent>
			{ (content == 'BEFORE') && 
				<SaleBefore sale={sale} onViewSaleClicked={() => this.switchContent('DURING')}/>
			}
			{ (content == 'DURING') && 
				<SaleDuring sale={sale} onLastOfferDone={() => this.switchContent('AFTER')} 
										onResetSaleClicked={() => this.switchContent('BEFORE')}
										onRevealSaleClicked={() => this.switchContent('AFTER')}/>
			}
			{ (content == 'AFTER') && 
				<SaleAfter sale={sale} onResetSaleClicked={() => this.switchContent('BEFORE')}/>
			}
			</CardContent>
		)
	}
