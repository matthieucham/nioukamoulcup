...
export class SalesList extends React.Component {
	...
	render() {
		...
		const cards = sales.map( (sal, index) => <SaleCard key={'salecard'+index} sale={sal} />);
		return (
			<div>
	...
	render() {
		...
		const releasesLines = releases.map( (rel, index) => 
			<div key={'releaseline'+index} className="line_table">
				<div className="cell_table"><a href={rel.signing.player.url}>{rel.signing.player.display_name}</a></div>
			...
			<div>
				<h2>Reventes de cette session ({releases.length})</h2>
				{
					...
					<div className="releases-list">
						<div className="header_table"><div className="cell_table">Joueur</div><div className="cell_table">Revendu par</div><div className="cell_table">Montant</div></div>
						{releasesLines}
					</div>
				}
				{
...
export class SolvedMerkatoSession extends React.Component {
	...
	render() {
		...
		return (
			<div>
				<h1>Session n°{session.number}</h1>
				<ul>
					<li>Enchères du {start} au {end}</li>
					{ bonus>0 && 
						<li>Bonification {bonusDisplay}</li> }
				</ul>
