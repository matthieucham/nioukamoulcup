...
export const ReleasesTable = ({ releases, height }) => {
		...
		return (<AutoSizer disableHeight>
			{({width}) => (
				...
				<Column	label="Date"
				dataKey="signing.end"
				cellDataGetter={({rowData}) => Moment(rowData.signing.end).format('DD/MM/YYYY') } 
				width={120}/>

				<Column	label="Joueur"
				dataKey="signing.player.display_name"
				cellDataGetter={({rowData}) => rowData.signing.player.display_name }
				cellRenderer={({rowData}) => <a href={rowData.signing.player.url}>{rowData.signing.player.display_name}</a> } 
