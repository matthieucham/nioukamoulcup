...
class TeamInfosTable extends Component {
	...
	render() {
		...
		return (
			...
			{({width}) => (
				...
				<Column	label="Nom"
				dataKey="name"
				cellRenderer={({rowData}) => <a href={rowData['url']}>{rowData['name']}</a> } 
				...
				<Column	label="Recrues"
				dataKey="current_signings"
				cellDataGetter={({rowData}) => this._countMissingRecrues(rowData) > 0 ? '-'+this._countMissingRecrues(rowData) : 0}
				width={50}
				disableSort/>
	...
	_countMissingRecrues(team) {
		let miss = 0;
		(['G', 'D', 'M', 'A']).forEach(pos => { miss += Math.max(0, team['attributes']['formation'][pos] - team['current_signings'][pos]) } )
		return miss
	}
